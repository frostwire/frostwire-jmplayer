...
static void scale(struct SwsContext *sws1, struct SwsContext *sws2, uint8_t *src[MP_MAX_PLANES], int src_stride[MP_MAX_PLANES],
    ...
    if(interlaced){
        ...
        sws_scale(sws1, (ConstUInt8PtrPtr) src2, src_stride2, y>>1, h>>1, dst2, dst_stride2);
        for(i=0; i<MP_MAX_PLANES; i++){
            src2[i] += src_stride[i];
            dst2[i] += dst_stride[i];
        }
        sws_scale(sws2, (ConstUInt8PtrPtr) src2, src_stride2, y>>1, h>>1, dst2, dst_stride2);
    }else{
        sws_scale(sws1, (ConstUInt8PtrPtr) src2, src_stride, y, h, dst, dst_stride);
    }
}
