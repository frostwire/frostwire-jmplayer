...
// gubatron, nov 5th 2025
typedef const uint8_t * const* ConstUInt8PtrPtr;
                       ...
                       int* stride, int w,int h, int x, int y)
{
    if (vf->priv->store_slices) {
      sws_scale(vf->priv->ctx, (ConstUInt8PtrPtr) src, stride, y, h, vf->priv->pic->data, vf->priv->pic->linesize);
    }
...
static void scale_image(struct vf_priv_s* priv, mp_image_t *mpi)
{
    ...
    sws_scale(priv->ctx, (ConstUInt8PtrPtr) mpi->planes, mpi->stride, 0, mpi->height, priv->pic->data, priv->pic->linesize);
}
